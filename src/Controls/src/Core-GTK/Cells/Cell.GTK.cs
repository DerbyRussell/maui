...
namespace Microsoft.Maui.Controls
{
	...
	public abstract class Cell_GTK : Element, ICellController, IFlowDirectionController, IPropertyPropagationController, IVisualController
	{
		public const int DefaultCellHeight = 40;
		public static readonly BindableProperty IsEnabledProperty = BindableProperty.Create("IsEnabled", typeof(bool), typeof(Cell_GTK), true, propertyChanged: OnIsEnabledPropertyChanged);
		...
		public IPlatformElementConfiguration<T, Cell_GTK> On<T>() where T : IConfigPlatform
		{
			return GetElementConfiguration().On<T>();
		}

		IElementConfiguration<Cell_GTK> GetElementConfiguration()
		{
			return _elementConfiguration.Value;
		}

		class ElementConfiguration : IElementConfiguration<Cell_GTK>
		{
			readonly Lazy<PlatformConfigurationRegistry<Cell_GTK>> _platformConfigurationRegistry;
			public ElementConfiguration(Cell_GTK cell)
			{
				_platformConfigurationRegistry =
					new Lazy<PlatformConfigurationRegistry<Cell_GTK>>(() => new PlatformConfigurationRegistry<Cell_GTK>(cell));
			}
			...
			internal PlatformConfigurationRegistry<Cell_GTK> Registry => _platformConfigurationRegistry.Value;
		}
		#endregion
